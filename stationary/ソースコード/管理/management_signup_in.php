...
if(isset($_POST['addre'])){
    $addre = $_POST['addre'];//入力した郵便番号からもとめた住所を変数に代入する
}else if(isset($_POST['user_address'])){
    $addre = $_POST['user_address'];//入力した住所を変数に代入する
}else{
    $addre="";//初期値
}
?>
...
<div id="app">
    <div class="container">
        ...
        <form action = "address_out.php" method= "get">
            ...
            <p class="search_btn_p"><button value = "send" onsubmit="return btnNumber()" onclick="return btnNumber()" class="search_btn">検索</button></p>
        </form>
        <form action="management_re_signup_in.php" method="post">
            <div class="range">
                <h2 class="subtitle">id</h2><p class="width_p"><span class="span">※文字種：アルファベット、数字(半角)の組み合わせ/
                        制限：ハイフン、*、@、/、(、)、<、>、$、#、&、%、!、ひらがな、カタカナ、漢字はなし/文字数：8文字</span></p>
                <p class="txt_information"><input type="text" class="text" name="user_id" id="id" v-model="id"></p>
                <p v-if="isInValidId" class="error_signup">制限を満たしていません。</p>

                <h2 class="subtitle">pass</h2><p class="width_p"><span class="span">※文字種：数字(半角)/制限:ハイフン、*、@、
                        /、(、)、<、>、$、#、&、%、!、アルファベット、ひらがな、カタカナ、漢字はなし/文字数：4文字</span></p>
                <p class="txt_information"><input type="password" class="text" name="user_pass" id="pass" v-model="pass"></p>
                ...
                <h2 class="subtitle">credit number</h2><p class="width_p"><span class="span">※文字種：数字/制限：ハイフンはなし/文字数：16文字</span></p>
                <p class="txt_information"><input type="number" class="number_text" name="credit_number" id="credit" v-model="credit"></p>
